...
export default function WizardForm({ session }: { session: Session | null }) {
  ...
  // Steps: 1 = Tempahan, 2 = Dokumen (KYC), 3 = Perjanjian, 4 = Pembayaran, 5 = Sukses, 6 = Active Booking Dashboard (legacy redirect, integrated in main dashboard tab 1 now)
  ...
  // Papan Pemuka Pengguna (Full User Dashboard) State
  ...
  const [userTab, setUserTab] = useState<'active' | 'history' | 'profile'>('active');
  const [isBookingMode, setIsBookingMode] = useState(false);

  // Initial load: fetch locations list
  useEffect(() => {
  ...
  const fetchActiveBooking = async (user: any, forceDefaultMode = false) => {
    ...
    try {
      ...
      if (borrower) {
        // 2. Fetch ALL bookings for this borrower to populate history tab
        const { data: bookings, error: histErr } = await supabase
          .from('bookings')
          .select('*')
        ...
        if (bookings) {
          ...
          if (latest) {
            if (latest.payment_status === 'Paid') {
              ...
              setIsBookingMode(false); // Render dashboard directly!
              setUserTab('active');
              ...
              setIsBookingMode(true);
              setStep(4); // Land on Payment screen to complete checkout!
            } else {
              // Latest booking is Cancelled/Finished, show dashboard empty active trip
              setActiveBooking(null);
              setIsBookingMode(false);
              setUserTab('active');
            }
          } else {
            // No bookings, show empty dashboard
            setActiveBooking(null);
            setIsBookingMode(false);
            setUserTab('active');
          }
        ...
        // Profile doesn't exist, show booking form Step 1
        setIsBookingMode(true);
        setStep(1);
      }
    } catch (err) {
  ...
  return (
    <div className="card" style={{ maxWidth: !isBookingMode && session ? '640px' : '600px' }}>
      
      {/* ─── FULL USER PORTAL / DASHBOARD VIEW ─── */}
      {!isBookingMode && session ? (
        ...
        /* ─── STANDARD WIZARD BOOKING FLOW STEPS 1-5 ─── */
        <div>
          {/* Stepper */}
          {step < 4 && (
